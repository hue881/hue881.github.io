...
function renderPlayers(){
  ...
  state.players.forEach((p,i)=>{
    ...
    d.innerHTML=`<span class="av">${p.avatar}</span><span class="nm">${escapeHtml(p.name)}</span><button class="rm" title="Remove" onclick="removePlayer(${i})">\u2715</button>`;
    wrap.appendChild(d);
  });
...
function renderDecks(){
  ...
  LESSONS.forEach(l=>{
    const d=document.createElement('div');
    d.className='deck';
    d.style.background=`linear-gradient(135deg,${l.color},${shade(l.color,-25)})`;
    d.dataset.id=l.id;
    d.innerHTML=`<span class="day">Day ${l.day}</span><div class="em">${l.emoji}</div><h3>${l.title}</h3><p>${l.subtitle}</p><div class="cnt">${l.questions.length} questions</div>`;
    d.onclick=()=>selectDeck(l.id,d);
    grid.appendChild(d);
  });
  // Mixed deck
  const m=document.createElement('div');
  m.className='deck mixed';
  m.dataset.id='mixed';
  const total=LESSONS.reduce((s,l)=>s+l.questions.length,0);
  m.innerHTML=`<span class="day">Mix</span><div class="em">\ud83c\udfb2</div><h3>All Lessons</h3><p>A random mix from every topic \u2014 the ultimate challenge.</p><div class="cnt">${total} questions in the pool</div>`;
...
function setupSegments(){
  ...
  document.querySelectorAll('#segCount button').forEach(b=>{
    b.onclick=()=>{
      ...
      b.classList.add('on');
      state.qCount=+b.dataset.n;
      if(typeof AudioFX!=='undefined') AudioFX.play('click');
    };
  });
  document.querySelectorAll('#segTime button').forEach(b=>{
    b.onclick=()=>{
      document.querySelectorAll('#segTime button').forEach(x=>x.classList.remove('on'));
      b.classList.add('on');
      state.timeLimit=+b.dataset.t;
      if(typeof AudioFX!=='undefined') AudioFX.play('click');
    };
  });
}
...
function buildMixedDeck(){
  let pool=[];
  LESSONS.forEach(l=>{
    pool=pool.concat(l.questions.map(q=>({...q, lessonId:l.id, lessonTitle:l.title, lessonColor:l.color, emoji:l.emoji})));
  });
  return pool;
}
function startGame(){
  ...
  if(state.qCount>0){
    ...
    rounds=Math.max(1, Math.floor(basePool.length/nPlayers));
  }
...
function showFeedback(correct,pts,q,player,timedOut,pickedIdx){
  ...
  if(correct){
    ...
    if(!timedOut && pickedIdx>=0){
      html+=`<div class="wrong-line">You picked <b>${escapeHtml(q.a[pickedIdx])}</b><br><span class="wrong-note">${escapeHtml(wrongExplainer(q,pickedIdx))}</span></div>`;
    } else if(timedOut){
    ...
    html+=`<div class="why-note">${escapeHtml(q.why)}</div>`;
    w.innerHTML=html;
  }

  fb.classList.add('show');
  // next button label
  document.getElementById('nextBtn').textContent = isLastPlayerOfRound()? "See Leaderboard" : "Next Player";
...
const TOPIC_NUDGE = {
  "Computational Thinking & Algorithms": "Think about what an algorithm is \u2014 a precise, ordered set of steps \u2014 and how AI can still make mistakes if the data or instructions are flawed.",
  "Scratch: Building Blocks": "Recall Scratch's building blocks: sequences run in order, events trigger scripts, loops repeat, conditionals branch, and variables store values.",
  "Python Basics": "Remember Python fundamentals: print() displays text, for loops repeat a fixed number of times, and turtle graphics needs turtle.done() to stay open.",
  "Python: Logic & Games": "Recall if/elif/else runs the first true branch and stops, lists store multiple items, and while True: needs a break to ever end.",
  "AI in Practice": "Think about the machine learning pipeline: training data \u2192 model \u2192 prediction, and how biased or limited data leads to unfair results.",
  "Capstone & Shipping Code": "Remember the debugging method: Stop, Check, Find, Fix one thing at a time, then Test \u2014 and that every capstone needs one clear AI idea."
};
...
function showScoreboard(){
  ...
  sorted.forEach((p,i)=>{
    ...
    row.innerHTML=`<div class="rk">${medal}</div><div class="av">${p.avatar}</div><div class="nm">${escapeHtml(p.name)}</div><div class="sc">${p.score}${p.lastDelta>0?`<span class="delta">+${p.lastDelta}</span>`:''}</div>`;
...
function showResults(){
  ...
  layout.forEach(item=>{
    ...
    el.innerHTML=`<div class="ava">${item.p.avatar}</div><div class="pname">${escapeHtml(item.p.name)}</div><div class="pscore">${item.p.score} pts</div><div class="bar">${item.rank===1?'\ud83e\udd47':item.rank===2?'\ud83e\udd48':'\ud83e\udd49'}</div>`;
    pod.appendChild(el);
  });
  ...
  sorted.slice(3).forEach((p,i)=>{
    ...
    row.innerHTML=`<div class="rk">${i+4}</div><div class="av">${p.avatar}</div><div class="nm">${escapeHtml(p.name)}</div><div class="sc">${p.score}</div>`;
...
let confetti=[];
function resizeC(){ cvs.width=innerWidth; cvs.height=innerHeight; }
...
function refreshSoundButtons(){
  ...
  if(mb){ mb.textContent = AudioFX.prefs.music ? '\ud83c\udfb5' : '\ud83c\udfb5\ufe0f\u0336'; mb.classList.toggle('off', !AudioFX.prefs.music); }
...
document.addEventListener('pointerdown', e=>{
  ...
  if(panel && panel.classList.contains('show') && wrap && !wrap.contains(e.target)) panel.classList.remove('show');
});
function toggleMusic(){
  AudioFX.resume();
  AudioFX.setMusic(!AudioFX.prefs.music);
  // restart the appropriate track if music was just turned on
...
function primeAudio(){
  ...
  if(!document.getElementById('screen-play').classList.contains('active')) AudioFX.playMusic('menu');
